...
const generateProducts = () => {
  const adjectives = ['Ultra', 'Pro', 'Lite', 'Advanced', 'Mini', 'Mega'];
  const brands = ['Alpha', 'BetaTech', 'GammaWare', 'DeltaElectro', 'Epsilon'];
  const productTypes = ['Phone', 'Laptop', 'Speaker', 'Camera', 'Headphones', 'Tablet'];
  ...
  const randomName = () => {
    ...
    return {
      name: `${adj} ${brand} ${product}`,
      productTypes: product,
    };
  };
  // generate 100 products
  return Array.from({ length: 100 }, (_, index) => {
    const productInfo = randomName();
    return {
      _id: new ObjectId(),
      ...
      price: Math.floor(Math.random() * 10_000_000) + 1_000_000,
      // stock from 3 to 20
      stock: Math.floor(Math.random() * 20) + 3,
      vendor: `00000001db42a4ac30ff65a${Math.random() > 0.5 ? 1 : 2}`,
    }
  });
}
